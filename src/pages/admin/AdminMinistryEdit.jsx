...
import { Save, Plus, Trash2, Video, Pencil, ArrowLeft } from 'lucide-react';
...
import { getMinistryById, updateMinistry } from '@/services/ministriesService';
import {
  getVideosForMinistry,
  createYouthVideo,
  updateYouthVideo,
  deleteYouthVideo,
} from '@/services/youthVideosService';
import { MINISTRY_SECTION_IDS, DEFAULT_MINISTRY_SECTIONS } from '@/constants/ministrySections';
...
function MinistryVideoEditor({ ministryId, video, trigger, onSaved }) {
  const { t } = useLanguage();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (video?.id) {
        ...
        await createYouthVideo({ ...data, ministryId });
      }
...
function MinistryVideosTab({ ministryId, ministrySlug }) {
  ...
  const refresh = () => {
    setLoading(true);
    const legacy = ministrySlug === 'youth-children';
    getVideosForMinistry(ministryId, { legacyYouthSlug: legacy })
      .then(setVideos)
      .catch(() => setVideos([]))
    ...
    <div className="space-y-4">
      <div className="flex justify-end">
        <MinistryVideoEditor
          ...
          trigger={<Button><Plus /> {t('admin.youth.newVideo')}</Button>}
          onSaved={refresh}
        />
      </div>
      {loading ? (
...
export default function AdminMinistryEdit() {
  ...
  const programs = useFieldArray({ control: form.control, name: 'ministries' });
  ...
  useEffect(() => {
    ...
    getMinistryById(ministryId)
      .then((m) => {
        if (!active) return;
        if (!m) {
          navigate(ROUTES.adminMinistries, { replace: true });
          return;
        }
        setMinistry(m);
        form.reset(m);
  ...
  const toggleSection = (sid) => {
    ...
    form.setValue('sections', DEFAULT_MINISTRY_SECTIONS.filter((id) => set.has(id)), { shouldDirty: true });
  };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await updateMinistry(ministryId, payload);
      await refresh();
      toast.success(t('admin.flash.saved'));
    ...
    <>
      <div className="mb-6">
        <Button asChild variant="ghost" size="sm" className="gap-1 px-0 text-muted-foreground hover:text-foreground">
          <Link to={ROUTES.adminMinistries}>
            <ArrowLeft className="h-4 w-4" /> {t('admin.ministries.backToList')}
          </Link>
        </Button>
      </div>
      ...
      <header className="mb-8">
        <h1 className="font-hero text-3xl font-semibold text-primary md:text-4xl">
          {ministry.navLabelEn || ministry.slug}
        </h1>
        <p className="mt-1 max-w-2xl text-sm text-muted-foreground">{t('admin.ministries.editSubtitle')}</p>
      </header>

      <div className="mb-6 inline-flex rounded-lg border border-border bg-card p-1">
        {[
          { id: 'settings', label: t('admin.ministries.tabs.settings') },
          { id: 'content', label: t('admin.ministries.tabs.content') },
          { id: 'videos', label: t('admin.ministries.tabs.videos') },
        ].map((it) => (
          <button
            key={it.id}
            type="button"
      ...
      {tab === 'videos' ? (
        <MinistryVideosTab ministryId={ministry.id} ministrySlug={ministry.slug} />
      ) : (
            ...
            <>
              <Card>
                <CardContent className="space-y-5 p-6">
                  <h2 className="font-display text-lg font-semibold text-primary">{t('admin.ministries.settingsNav')}</h2>
                  <Field label={t('admin.ministries.field.slug')}>
                    <Input {...form.register('slug', { required: true })} />
                  </Field>
                  <BilingualField label={t('admin.ministries.field.navLabels')} {...enAm('navLabel')} register={form.register} />
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label={t('admin.ministries.field.sortOrder')}>
                      <Input type="number" {...form.register('sortOrder', { valueAsNumber: true })} />
                    </Field>
                    <div className="flex flex-col gap-2">
                      <Label>{t('admin.ministries.field.visible')}</Label>
                      <Controller
                        name="enabled"
                        control={form.control}
                        render={({ field }) => (
                          <Switch checked={field.value !== false} onCheckedChange={(v) => field.onChange(v)} />
                        )}
                      />
                    </div>
                  </div>
              ...
              <Card>
                <CardContent className="space-y-4 p-6">
                  <h2 className="font-display text-lg font-semibold text-primary">{t('admin.ministries.sectionsTitle')}</h2>
                  <p className="text-sm text-muted-foreground">{t('admin.ministries.sectionsHelp')}</p>
                  <div className="grid gap-3 sm:grid-cols-2">
                    {MINISTRY_SECTION_IDS.map((sid) => (
                      <label key={sid} className="flex cursor-pointer items-center gap-3 rounded-md border border-border bg-muted/20 p-3">
                        <Checkbox
                          checked={(form.watch('sections') || []).includes(sid)}
                          onCheckedChange={() => toggleSection(sid)}
                        />
                        <span className="text-sm font-medium">{t(`admin.ministries.section.${sid}`)}</span>
                      </label>
                    ))}
                  </div>
                </CardContent>
              ...
              <Card>
                <CardContent className="space-y-5 p-6">
                  <div className="flex items-center justify-between">
                    ...
                    <Button type="button" variant="outline" size="sm" onClick={() => programs.append({ titleEn: '', titleAm: '', bodyEn: '', bodyAm: '' })}>
                      <Plus /> {t('admin.add')}
                    </Button>
                  </div>
                  {programs.fields.map((field, idx) => (
                    <div key={field.id} className="rounded-md border border-border bg-muted/30 p-4">
                      <div className="flex items-center justify-between">
                        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">#{idx + 1}</p>
                        <Button type="button" variant="ghost" size="icon-sm" onClick={() => programs.remove(idx)}>
                          <Trash2 />
                        </Button>
                      </div>
